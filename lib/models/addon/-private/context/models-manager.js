...
export default class ModelsManager extends Destroyable {
  ...
  createNewInternalModel(opts) {
    ...
    if(modelType === 'persisted') {
      ...
      // id or path?
      internal = new InternalTransientModel(context, modelClass, path, data);
    }

    // create might have duplicate persistentPath
    // transient models should assert that
  ...
  existingInternalModel(opts) {
    ...
    // split identity into persisted and transient
    // transient identity per normalized name -> per id
