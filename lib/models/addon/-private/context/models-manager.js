...
export default class ModelsManager extends Destroyable {
  ...
  _modelClassForName(modelName) {
    ...
    assert(`model for name '${normalizedName}' is not registered`, !!factory);
    return { factory, normalizedName, factoryName };
  }

  modelClassForName(modelName) {
    let info = this._modelClassForName(modelName);
    if(!this.modelNameForClass(info.factory.class)) {
      set(info.factory.class, 'modelName', info.normalizedName);
    }
    return info.factory;
  }
