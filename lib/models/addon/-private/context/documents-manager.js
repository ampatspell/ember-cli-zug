...
export default EmberObject.extend({
  ...
  _createLoadedInternalDocument(ref, data_) {
    let path = ref.path;
    let context = this.context._internal;
    let identity = context.identity.documents;
    let internal = identity.persisted(path);
  ...
  createLoadedDocument(ref, data) {
    return this._createLoadedInternalDocument(ref, data).model(true);
  },
  ...
  // loadFirstDocument should have an option to load {exists: false } documents
  // This needs to create persisted reference: PersistedDocumentReference, PersistedQueryReference, PersistedCollectionReference
  loadFirstDocument(selector) {
    selector = parseSelector(selector, this.context._internal.firestore);
    if(selector.type === 'path') {
      return this.loadExistingDocument(selector.opts);
    } else if(selector.type === 'document-ref') {
      // let ref = selector.ref;
    } else if(selector.type === 'collection-ref') {
      // let ref = selector.ref.limit(1);
    }
