...
export default class DocumentsManager extends Destroyable {
  ...
  createPersistedReferenceWithRef(ref, exists) {
    return new PersistedReference(ref, exists);
  }
  ...
  createInternalDocument(reference, data) {
  ...
  existingInternalDocument(opts) {
    ...
    if(!internal && opts.create) {
      ...
      let data = this.createInternalData(opts.data);
      internal = this.createInternalDocument(reference, data);
