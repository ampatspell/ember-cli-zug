...
export default class DataManager extends Destroyable {
  ...
  update(current, value, format, notify) {
    ...
    if(currentSerializer === valueSerializer) {
      internal = currentSerializer.update(current, value, format, notify);
      replace = currentSerializer.updateReplacesContent;
    } else {
      internal = valueSerializer.create(value, format);
      replace = true;
    }
