...
export default class InternalContext extends Internal {
  ...
  modelNameForDocument(document) {
    let modelName = this.opts.modelNameForDocument(document.model(true), this.model(true));
    assert(`opts.modelNameForDocument must return model name`, typeof modelName === 'string' && modelName.trim().length);
    return modelName;
  }
