...
const serialized = () => computed('id', 'collection', 'path', 'isNew', 'isDirty', 'isLoaded', 'isSaving', 'isExisting', 'isError', 'error', 'data.serialized', function() {
  let data = this.get('data.serialized');
  let ref = this.getProperties('id', 'path', 'collection');
  let state = this.getProperties('isNew', 'isDirty', 'isLoaded', 'isSaving', 'isExisting', 'isError', 'error');
  return {
    ref,
    state,
    data
